#pineconeに保存されたベクトルデータcontentのweightを1に更新するスクリプトです。
...
def get_vector_ids_and_update_weight(page_size=50, page_number=1):
    ...
    while True:
     ...
     # 更新処理スタート
    print("\n=== type='content' のベクトルをすべて weight=1 に更新します ===")
    batch_size = 100
    ids_to_update = []
    ...
    for vid in ids_to_update:
        try:
            index.update(id=vid, set_metadata={"weight": 1})
            print(f"✅ {vid}: 更新完了（weight → 1）")
        except Exception as e:
            print(f"❌ {vid}: 更新失敗 - {e}")
